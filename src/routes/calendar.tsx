      ...
      <div className="grid gap-6 lg:grid-cols-3">
        ...
        <div className="lg:col-span-2 rounded-2xl border border-border/50 bg-card/45 p-5 shadow-card flex flex-col justify-between">
          ...
          <div className="mt-6 pt-4 border-t border-border/40 text-xs text-muted-foreground flex gap-4 justify-center">
            <span className="flex items-center gap-1"><span className="size-2 rounded-full bg-warning" /> Agendado</span>
            <span className="flex items-center gap-1"><span className="size-2 rounded-full bg-success" /> Publicado</span>
            <span className="flex items-center gap-1"><span className="size-2 rounded-full bg-destructive" /> Falhou</span>
          </div>
        </div>
            ...
            {loading ? (
              ...
              <div className="space-y-4 max-h-[350px] overflow-y-auto pr-1.5">
                {postsOnSelectedDay.map(post => {
                  const statusLabel = {
                    pending: { text: "Agendado", cls: "bg-warning/15 text-warning border-warning/30" },
                    published: { text: "Publicado", cls: "bg-success/15 text-success border-success/30" },
                    failed: { text: "Falhou", cls: "bg-destructive/15 text-destructive border-destructive/30" }
                  };
                  const st = statusLabel[post.status] || { text: "Pendente", cls: "bg-secondary text-foreground" };
                  
                  return (
                    <div key={post.id} className="p-3.5 rounded-xl bg-card border border-border/50 hover:bg-card/75 transition shadow-sm flex gap-3 relative group">
                      <video src={post.video_url} className="size-16 rounded-lg object-cover bg-background shrink-0" muted />
                      <div className="min-w-0 flex-1 flex flex-col justify-between py-0.5">
                        <div>
                          <div className="flex items-center gap-1.5 text-[10px]">
                            <strong className="text-primary truncate">@{post.instagram_accounts?.username || "instagram"}</strong>
                        ...
                        <div className="flex items-center gap-2 mt-2">
                          <span className={`inline-flex items-center text-[9px] font-bold border px-1.5 py-0.5 rounded-full ${st.cls}`}>
                            {st.text}
                          </span>
                        </div>
                      </div>
          ...
          {accounts.length === 0 && (
            <div className="rounded-xl border border-warning/30 bg-warning/5 p-4 flex gap-3 text-xs mt-6">
              ...
              <p className="text-muted-foreground">
                Você deve <Link to="/accounts" className="text-warning underline font-semibold">conectar uma conta do Instagram</Link> na aba de Contas antes de poder agendar seus Reels.
              </p>
            </div>
          )}
        </div>
        ...
        <DialogContent className="sm:max-w-4xl bg-card border border-border/60 max-h-[90vh] overflow-y-auto rounded-2xl grid md:grid-cols-2 gap-6 p-6">
          ...
          <div className="space-y-5">
            ...
            <form onSubmit={handleScheduleSubmit} className="space-y-4">
                  ...
                  <div className="rounded-xl border-2 border-dashed border-border/80 hover:border-primary/60 bg-secondary/10 hover:bg-secondary/20 transition-all p-5 text-center flex flex-col items-center justify-center min-h-[120px]">
                    {videoFile ? (
                      <div className="flex flex-col items-center gap-2 text-foreground">
                        ...
                        <span className="font-semibold text-xs truncate max-w-[250px]">{videoFile.name}</span>
                        <span className="text-[10px] text-muted-foreground">
                          {(videoFile.size / 1024 / 1024).toFixed(1)} MB
                        </span>
                        <span className="text-[10px] text-primary underline mt-1 font-medium">Trocar arquivo</span>
                      </div>
                    ) : (
                      <div className="flex flex-col items-center gap-1.5 text-muted-foreground">
                        <Upload className="size-6 text-muted-foreground/60" />
                        <span className="text-xs font-semibold text-foreground">Clique para enviar um vídeo</span>
              ...
              {/* Seletor de Conta */}
              <div className="space-y-2">
                <Label htmlFor="modalAccount" className="text-sm font-bold">Conta do Instagram</Label>
                <Select value={accountId} onValueChange={setAccountId} required>
                  <SelectTrigger id="modalAccount" className="bg-secondary/40 border-border/60 rounded-xl h-10 font-medium">
              ...
              <div className="space-y-2">
                <Label htmlFor="modalScheduled" className="text-sm font-bold">Data e Hora de Publicação</Label>
                <Input
                  id="modalScheduled"
                  type="datetime-local"
                  required
              ...
              {/* Legenda (Caption) */}
              <div className="space-y-2">
                <Label htmlFor="modalCaption" className="text-sm font-bold">Legenda do Post</Label>
                <Textarea
                  id="modalCaption"
                  value={caption}
                ...
                <div className="relative w-full h-full bg-neutral-950 flex items-center justify-center group/video">
                  ...
                  <div className="absolute bottom-5 left-4 right-10 text-white z-15 space-y-2 pointer-events-none">
                    ...
                    <p className="text-[11px] leading-relaxed line-clamp-3 text-neutral-100/90 font-medium">
                      {caption || "A legenda do seu Reels aparecerá aqui quando você começar a digitar no formulário ao lado..."}
                    </p>
                  </div>
                  ...
                  <div className="absolute bottom-16 right-2 text-white z-15 flex flex-col gap-4 items-center pointer-events-none">
                    <div className="flex flex-col items-center gap-0.5"><span className="text-base">❤️</span><span className="text-[8px]">Curtir</span></div>
                    <div className="flex flex-col items-center gap-0.5"><span className="text-base">💬</span><span className="text-[8px]">Comentar</span></div>
                    <div className="flex flex-col items-center gap-0.5"><span className="text-base">✈️</span><span className="text-[8px]">Enviar</span></div>
                  </div>
